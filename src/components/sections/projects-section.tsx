	...
	if (!images.length) {
		return (
			<div className="h-full w-full bg-muted flex items-center justify-center">
				<span className="font-pixel text-[10px] text-muted-foreground tracking-widest">&lt;NO IMG/&gt;</span>
			</div>
		);
	}
		...
		<>
			...
			>
				...
				<div
					...
					aria-hidden="true"
					style={{
						backgroundImage: 'repeating-linear-gradient(0deg, oklch(0% 0 0 / 0.12) 0px, oklch(0% 0 0 / 0.12) 1px, transparent 1px, transparent 3px)',
					}}
				/>
				...
				<div className="absolute top-3 right-3 z-10 opacity-0 group-hover/car:opacity-100 transition-opacity">
					<span className="font-pixel text-[7px] bg-black/85 text-primary px-2 py-1 border border-primary/60 tracking-widest">
						{images.length > 1 ? `${idx + 1}/${images.length} · VIEW ALL` : 'VIEW'}
					</span>
				</div>
			</div>
					...
					>
						...
						>
							<span className="font-pixel text-[9px] text-primary tracking-widest">
								{title.toUpperCase()} — GALLERY ({images.length} SCREENSHOTS)
							</span>
							<button
								onClick={() => setLightbox(false)}
	...
	return (
		...
		>
			<div className={`flex flex-col lg:flex-row lg:min-h-[480px] ${isEven ? '' : 'lg:flex-row-reverse'}`}>
				...
				{/* ── Content ── */}
				<div className={`flex flex-col flex-1 p-6 lg:p-8 gap-5 min-w-0 ${isEven ? 'lg:border-l' : 'lg:border-r'} border-dotted border-border`}>
					...
					<div>
						...
						<div className="flex flex-wrap gap-x-4 gap-y-1.5">
							{project.period && (
								<span className="font-pixel text-[7px] text-muted-foreground tracking-wide">{project.period}</span>
							)}
							{project.team && (
								<span className="font-pixel text-[7px] text-muted-foreground tracking-wide">· {project.team}</span>
							)}
						</div>
						{project.role && (
							<p className="font-pixel text-[7px] text-secondary mt-1 tracking-wide">{project.role}</p>
						)}
					</div>
					...
					{/* Tech stack badges */}
					<div className="flex flex-wrap gap-2" role="list" aria-label={`Technologies in ${project.title}`}>
						{project.techStack.map((tech, i) => {
							const { color, bg } = BADGE_PALETTE[i % BADGE_PALETTE.length];
							return (
								<span
									key={tech}
									role="listitem"
...
export const ProjectsSection = () => {
	...
	return (
		<section id="projects-section" className="py-16 sm:py-24 px-4 max-w-7xl mx-auto">
			...
			>
				<p className="font-pixel text-[10px] text-primary mb-3 tracking-widest">PROJECTS</p>
