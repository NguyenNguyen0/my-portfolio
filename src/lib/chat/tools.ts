...
function findProjectRepo(name: string) {
	...
	const p = projects.find(
		(proj) =>
			proj.title.toLowerCase().includes(norm) ||
			proj.id.toLowerCase() === norm,
	);
	if (!p) return null;
	if (p.githubUrl) {
...
const changeThemeTool = tool({
	description:
		'Switch the portfolio between DARK mode and LIGHT mode only. Do NOT use this to change colors — use change_accent_color for that.',
...
const changeAccentColorTool = tool({
	description:
		'Change the portfolio primary/accent color. Use when the user asks to change color, primary color, accent color, or "màu" in Vietnamese. Available: yellow (default), blue, purple, green, red, cyan, orange.',
	inputSchema: z.object({
		color: z.enum([
			'yellow',
			'blue',
...
const setHeroDescriptionTool = tool({
	...
	inputSchema: z.object({
		text: z
			...
			.max(200)
			.describe(
				'New description text for the hero section (max 200 chars)',
			),
	}),
